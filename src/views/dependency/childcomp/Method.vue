<template>
  <div v-show="this.show" :data="graph_data">
    <el-button type="primary" icon="el-icon-share" round size="mini" @click="methodGraph">生成Method依赖</el-button>
    <el-input v-model="search" size="mini" placeholder="Input Method Name" @input="handleSearch" style="margin-left: 40px;margin-right: -80px;width: 150px"></el-input>
    <div id="dep_graph" style="height: 470px; width: 100%"></div>
    <div id='edge_path' style="height:15px; width:500px;color:darkorange;font-size: 10px;margin-top: -15px;margin-left: -10px"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    methodGraph() {
      //给每个节点写入itemStyle  还要写入ignore,flag,draggble都为true
      ...
      let option = {
            ...
            dataView: {
              ...
              optionToContent: function(opt) {
                ...
                let table = '<table border="1" style="margin-left:20px;border-collapse:collapse;font-size:12px;text-align:center"><tbody><tr class="dataViewTr">'
                    ...
                    + '<td style="font-weight: bolder; font-size: 15px">isConstructor</td>'
                    + '<td style="font-weight: bolder; font-size: 15px">codeSnippet</td>'
                    + '</tr>';
                for (let i = 0, l = series.length; i < l; i++) {
                    table += '<tr>'
                        + '<td>' + series[i].id + '</td>'
                        + '<td>' + series[i].simpleName + '</td>'
                        + '<td>' + series[i].isConstructor + '</td>'
                        + '<td>' + series[i].codeSnippet + '</td>'
                        + '</tr>';
                }
                table += '</tbody></table>';
                return table;
        ...
        series: [{
          ...
          force: {
            repulsion: 75,//节点之间的斥力因子。支持数组表达斥力范围，值越大斥力越大。
    ...
    handleSearch(value) {
      ...
      //data.simpleName.toLowerCase().includes(this.str.toLowerCase())
      for (let m = 0; m < nodesOption.length;m++) {
          if ((nodesOption[m].simpleName.indexOf(value) !== -1 && nodesOption[m].simpleName.indexOf(value) !== 0)){
            //找到对应的节点,改变对应节点的配置
            option.series[0].data[m].itemStyle.opacity = 1;
            option.series[0].data[m].itemStyle.shadowColor = "darkOrange";
                ...
                if (linksOption[k].source == nodesOption[m].id) {
                  ...
                  nodesOption.forEach(item => {
                    if(item.id == linkedNode){
                      ...
                      item.itemStyle.shadowColor = "darkOrange";
          ...
          } else if(nodesOption[m].simpleName.indexOf(value) == 0){
            //说明输入框为空，恢复到原来的配置
