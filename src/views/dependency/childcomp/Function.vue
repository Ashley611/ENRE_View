<template>
  <div v-show="this.show" :data="graph_data">
    <el-button type="primary" icon="el-icon-share" round size="mini" @click="funcGraph">生成Function依赖</el-button>
    <el-input v-model="search" size="mini" placeholder="Input Function Name" @input="handleSearch" style="margin-left: 40px;margin-right: -80px;width: 150px"></el-input>
    <div id="dep_graph" style="height: 470px; width: 100%"></div>
    <div id='edge_path' style="height:15px; width:500px;color:darkblue;font-size: 10px;margin-top: -15px;margin-left: -10px"></div>
  </div>
</template>
    ...
    handleSearch(value) {
      ...
      for (let m = 0; m < nodesOption.length;m++) {
        if ((nodesOption[m].simpleName.includes(value)) && value !== ""){
          ...
          option.series[0].data[m].itemStyle.shadowColor = "skyBlue";
              ...
              if (linksOption[k].source == nodesOption[m].id) {
                ...
                nodesOption.forEach(item => {
                  if(item.id == linkedNode){
                    ...
                    item.itemStyle.shadowColor = "skyBlue";
