<template>
  <h2 v-if="this.show === false">未上传相关历史依赖文件，无法获取信息</h2>
  <div v-else>
...
export default {
  ...
  methods: {
    getParams() {
      //获取对应文件
      let his_dep_file = this.$store.state.his_dep_file;
      //获取到的文件如果为空，将data的show设置为false,展示时如果show的值为false，就显示“未上传相关的历史依赖文件”
      if(his_dep_file.originalname == undefined) {
        //未上传文件
        this.show = false;
      } else {
        this.show = true;
        //向后台发送请求读取文件，获取文件内容
        dependency.find_his_dep_file(his_dep_file).then(r => {
          //在这里对文件格式重新修改，方便展示
           let finalFile = this.rewriteFile(r.data);
            //绘制图表
            this.historyGraph(finalFile)
            }).catch(e => {
          console.log(e)
    ...
    rewriteFile(file) {
      ...
      // itemObj.forEach(function (value, key, map){
      //   item[key] = value;
      // });
